      ...
      {(provided, snapshot) => {
        const cardContent = (
          <div
            ...
            className={`p-4 rounded-xl border select-none flex flex-col justify-between overflow-hidden group ${
              snapshot.isDragging
                ? "shadow-2xl ring-2 ring-indigo-500 bg-white opacity-95 cursor-grabbing z-[9999]"
                : "cursor-pointer"
            } ${
              isLockedByMe
                ? "border-indigo-500/90 bg-indigo-50/70 shadow-md shadow-indigo-500/10 ring-2 ring-indigo-500/30"
                : isLockedByOther
                  ? "border-amber-400/90 bg-amber-50/70 opacity-90 border-dashed ring-1 ring-amber-400/30 cursor-not-allowed"
                  : "border-slate-200/90 bg-white hover:border-indigo-300 shadow-xs hover:shadow-md"
            }`}
          >
            <div>
